...
企画書 §13。P0 完了（2026-09-18）。P1 はエミュレータで本番 Vision API による誕生まで確認済み（実機確認のみ残り）。P2 育成はエミュレータ＋疑似歩数で確認済み（Health Connect の実歩数は実機待ち）。P3 アート: **写真参照の個体生成（品質 low、性格で表情が変わる）** を本番で確認済み。共有バケットは失敗時の保険。現在: **P4 バトル**（模擬戦をエミュレータで通し確認済み、本番デプロイ済み。フレンド戦は 2 アカウント目での確認が残り）。

バトルのダメージは §6.2 の攻÷守ではなく 攻÷(攻＋守) × レベル係数（`shared-config/constants.json` の `_battleDamageDoc`）。Lv1 の初期値では攻÷守が 10 倍を超えて一撃で終わるため。
