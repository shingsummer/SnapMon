...
バトルのダメージ式は企画書 §6.2（v1.3 で改訂済み: 攻÷(攻＋守) × レベル係数 × 段階補正、会心は運÷3000 で 2 倍）。定数を触ったら `cd functions && npm run sim:battle` で検算し、§6.2 の表を更新する。技は Lv10/20 で自動習得（Lv30/40 の入れ替えは v1.1）。
