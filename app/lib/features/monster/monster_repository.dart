...
int remainingSnaps(Map<String, dynamic>? user, int snapsPerDay) {
  ...
  if (daily == null) return snapsPerDay;
  final today = jstDateKey(DateTime.now());
  if (daily['date'] != today) return snapsPerDay;
  final used = ((daily['snapsUsed'] as num?) ?? 0).toInt();
  return (snapsPerDay - used).clamp(0, snapsPerDay);
}
